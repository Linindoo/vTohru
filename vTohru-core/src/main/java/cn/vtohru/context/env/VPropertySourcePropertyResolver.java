...
public class VPropertySourcePropertyResolver implements PropertyResolver {

    private static final Logger LOG = ClassUtils.getLogger(VPropertySourcePropertyResolver.class);
    ...
    private static final VPropertySourcePropertyResolver.PropertyCatalog[] CONVENTIONS = {VPropertySourcePropertyResolver.PropertyCatalog.GENERATED, VPropertySourcePropertyResolver.PropertyCatalog.RAW};
    ...
    /**
     * Creates a new, initially empty, {@link VPropertySourcePropertyResolver} for the given {@link ConversionService}.
     *
     * @param conversionService The {@link ConversionService}
     */
    public VPropertySourcePropertyResolver(ConversionService<?> conversionService) {
        this.conversionService = conversionService;
        this.propertyPlaceholderResolver = new DefaultPropertyPlaceholderResolver(this, conversionService);
    }

    /**
     * Creates a new, initially empty, {@link VPropertySourcePropertyResolver}.
     */
    public VPropertySourcePropertyResolver() {
        this(ConversionService.SHARED);
    }

    /**
     * Creates a new {@link VPropertySourcePropertyResolver} for the given {@link PropertySource} instances.
     *
     * @param propertySources The {@link PropertySource} instances
     */
    public VPropertySourcePropertyResolver(PropertySource... propertySources) {
        this(ConversionService.SHARED);
     ...
     * @return This {@link VPropertySourcePropertyResolver}
     */
    public VPropertySourcePropertyResolver addPropertySource(@Nullable PropertySource propertySource) {
     ...
     */
    public VPropertySourcePropertyResolver addPropertySource(String name, @Nullable Map<String, ? super Object> values) {
    ...
    public boolean containsProperty(@Nullable String name) {
        if (StringUtils.isEmpty(name)) {
            ...
            if (result == null) {
                ...
                for (VPropertySourcePropertyResolver.PropertyCatalog convention : CONVENTIONS) {
    ...
    public boolean containsProperties(@Nullable String name) {
        if (!StringUtils.isEmpty(name)) {
            for (VPropertySourcePropertyResolver.PropertyCatalog propertyCatalog : CONVENTIONS) {
    ...
    public Collection<String> getPropertyEntries(@NonNull String name) {
        if (!StringUtils.isEmpty(name)) {
            Map<String, Object> entries = resolveEntriesForKey(
                    name, false, VPropertySourcePropertyResolver.PropertyCatalog.NORMALIZED);
            if (entries != null) {
                String prefix = name + '.';
    ...
    public @NonNull Map<String, Object> getProperties(String name, StringConvention keyFormat) {
        if (!StringUtils.isEmpty(name)) {
            Map<String, Object> entries = resolveEntriesForKey(name, false, keyFormat == StringConvention.RAW ? VPropertySourcePropertyResolver.PropertyCatalog.RAW : VPropertySourcePropertyResolver.PropertyCatalog.GENERATED);
            if (entries != null) {
                if (keyFormat == null) {
                ...
                entries = resolveEntriesForKey(name, false, VPropertySourcePropertyResolver.PropertyCatalog.GENERATED);
                if (keyFormat == null) {
                    keyFormat = StringConvention.RAW;
                }
                if (entries == null) {
    ...
    public <T> Optional<T> getProperty(@NonNull String name, @NonNull ArgumentConversionContext<T> conversionContext) {
        if (StringUtils.isEmpty(name)) {
            ...
            if (cached != null) {
                ...
                Map<String, Object> entries = resolveEntriesForKey(name, false, VPropertySourcePropertyResolver.PropertyCatalog.RAW);
                if (entries == null) {
                    entries = resolveEntriesForKey(name, false, VPropertySourcePropertyResolver.PropertyCatalog.RAW);
                }
                if (entries != null) {
                    ...
                    if (value == null) {
                        ...
                        if (value == null && name.indexOf('[') == -1) {
                            ...
                            Map<String, Object> rawEntries = resolveEntriesForKey(name, false, VPropertySourcePropertyResolver.PropertyCatalog.RAW);
            ...
            for (String property : properties) {
                ...
                for (String resolvedProperty : resolvedProperties) {
                    ...
                    if (i > -1) {
                        ...
                        Map<String, Object> entries = resolveEntriesForKey(propertyName, true, VPropertySourcePropertyResolver.PropertyCatalog.GENERATED);
                        if (entries != null) {
                        ...
                        if (first) {
                            Map<String, Object> normalized = resolveEntriesForKey(resolvedProperty, true, VPropertySourcePropertyResolver.PropertyCatalog.NORMALIZED);
                            if (normalized != null) {
                        ...
                        Map<String, Object> entries = resolveEntriesForKey(resolvedProperty, true, VPropertySourcePropertyResolver.PropertyCatalog.GENERATED);
                        if (entries != null) {
                ...
                final Map<String, Object> rawEntries = resolveEntriesForKey(property, true, VPropertySourcePropertyResolver.PropertyCatalog.RAW);
                if (rawEntries != null) {
     ...
     */
    @SuppressWarnings("MagicNumber")
    protected Map<String, Object> resolveEntriesForKey(String name, boolean allowCreate, @Nullable VPropertySourcePropertyResolver.PropertyCatalog propertyCatalog) {
        if (name.length() == 0) {
            return null;
        }
    ...
    private Map<String, Object>[] getCatalog(@Nullable VPropertySourcePropertyResolver.PropertyCatalog propertyCatalog) {
        propertyCatalog = propertyCatalog != null ? propertyCatalog : VPropertySourcePropertyResolver.PropertyCatalog.GENERATED;
